...
STAR_LIST_INTERNAL_TIME = 0.1  # お気に入りの検索時間の間隔
...
def cart_refresh():
    ...
    response = buy_session.get('https://www.bookoffonline.co.jp/spf-api2/goods_souko/cart/{}/{}'.format(cartNo, memNo[0]), timeout=3.0)
    ...
    while True:
        ...
        if (not cart_refreshing):
            ...
            if ((next_process_star == index) and (time.time() - wait_time) > STAR_LIST_INTERNAL_TIME):
                ...
                select_memNo = memNo[index % 2]
                try:
                    response = star_session.get(MAIN_URL + '/spf-api2/goods_souko/bookmark/' + select_memNo)  # エラー
                except Exception as e:
                    log.logger.exception(f'{e}')
                    continue
                ...
                if lock.locked():
                    lock.release()
            elif (time.time() - wait_time) > (STAR_LIST_INTERNAL_TIME + 0.1):
                wait_time = time.time()
                next_process_count(index)
